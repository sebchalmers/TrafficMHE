...
Q = {'rho': 1e0, 'v': 1e0, 'Ve': 1e1, 'alpha' : 1e2, 'beta' : 1e2}
A = {'rho': 1e2, 'v': 1e2}
...
for key in Costs.keys():
    for i in range(T.NumSegment): 
        ...
        if key == 'Stage':
            ...
            Costs[key] += 1e1*(T.VSpace['Inputs']['alpha',i] + T.VSpace['Inputs']['beta',i] - 1)**2

            #Slack weight
            Costs[key] += Q['alpha']*(T.VSpace['Slacks']['Salpha',i] + 5)**2
            Costs[key] += Q['beta']*(T.VSpace['Slacks']['Sbeta', i]  + 5)**2
