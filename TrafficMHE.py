...
for key in Costs.keys():
    for i in range(T.NumSegment): 
        ...
        if not(key == 'Terminal'):
            Ve_arg  = -(1/a)*(((1 + T.VSpace['Inputs']['alpha',i]))**a)*(T.VSpace['States']['rho',i]/rho_cr)**a
        ...
        if key == 'Stage': #Operative at time 1:N-1
            ...
            Costs[key] += 1e-4*dalpha**2
            Costs[key] += 1e-4*dbeta**2
        ...
        if key == 'Arrival': #Operative at time 0
            #Costs[key] += (T.VSpace['Inputs']['alpha',i])**2
            #Costs[key] += (T.VSpace['Inputs']['beta',i]-1)**2        
            ...
            Costs[key] += Q[ 'Ve']*(T.VSpace['Inputs']['Ve',  i] - T.VSpacePrev['Inputs']['Ve',  i])**2
