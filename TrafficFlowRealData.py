...
def _setSolver(self, V,Cost,g,P, Tol):
    ...
    if ('IneqConst' in g.keys()):
        lbg['IneqConst'] = -inf
        ubg['IneqConst'] =  0
    EqConst          = list(veccat(g.i['DynConst'  ]))+list(veccat(g.i['LiftConst'  ]))
...
class FreeWay:
    ...
    def __init__(self, Path = [], Slacks = [], Meas = [], ExtParam = []):
        ...
        VarList = {
                    'States': ['rho', 'v'],
                    'Inputs': ['r','f','alpha','beta', 'theta', 'Ve']
                  }
    ...
    def BuildMHE(self, Horizon = 1, SimTime = 1, Tol = 1e-3):
        ...
        for k in range(0,Horizon-1):
            ...
            for i in range(1,self.NumSegment-1):
                ...
                DynConst.append(  veccat(self.V['States',k+1,...,i]) - Xplus )
    ...
    def GenerateData(self, VStruct = [], EPStruct = [], ExtParam = [], Simulated = False, AddNoise = False, Accident = []):
        """
        ...
        TimeRange = len(VStruct['States',:,...,veccat])
        EP   = self._setParameters(EP = EPStruct, ExtParam = ExtParam)
        
        
        #Assign parameters (for Ve construction)
        ...
        if Simulated:
            ...
            for i in range(1,self.NumSegment-1):
                Data['States',0,'v',i]   = rand.normalvariate(100,   0)
                Data['States',0,'rho',i] = rand.normalvariate( 20,   0)
            
            for k in range(TimeRange-1):
                for i in [0,self.NumSegment-1]:
                    Data['States',k,'v',i]   = rand.normalvariate(100,   0)
                    Data['States',k,'rho',i] = rand.normalvariate( 20,   0)

            #Generate accident
            FaultySegment = 4
            if not(Accident == []):
                Data['Inputs',Accident,'alpha' ,FaultySegment] = 0.1
                Data['Inputs',Accident, 'beta' ,FaultySegment] = 0.9


            for k in range(TimeRange-1):                   
                for i in range(1,self.NumSegment-1):
                    #Assign theta
                    Data['Inputs',k, 'theta',i] = (1. + Data['Inputs',k,'alpha',i])**a
                    
                    #Assign Ve
                    Ve_arg  = -(1/a)*Data['Inputs',k,'theta',i]*(Data['States',k,'rho',i]/rho_cr)**a
                    Data['Inputs',k,'Ve',i] = vfree*exp(Ve_arg)
                    ...
                    if AddNoise:
                        StateNoise = np.array([
                                                rand.normalvariate(0,1e-2*np.mean(Data['States',k,'rho',:])),
                                                rand.normalvariate(0,1e-2*np.mean(Data['States',k,'v',:]))
                                              ])
        ...
        #Assign Ve on the boundaries (for consistancy)
        for k in range(TimeRange-1):
                for i in [0,self.NumSegment-1]:
                    Ve_arg  = -(1/a)*Data['Inputs',k,'theta',i]*(Data['States',k,'rho',i]/rho_cr)**a
                    Data['Inputs',k,'Ve',i] = vfree*exp(Ve_arg)
